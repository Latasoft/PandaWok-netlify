...
const ReservasPorGrupo = () => {
  const [datos, setDatos] = useState<GrupoReserva[]>([]);
  ...
  const fetchDatos = async () => {
    ...
    try {
      ...
      setError('Error al cargar las reservas por grupo');
    } finally {
      setLoading(false);
    }
  };

  const coloresBarras = ['#8B5CF6', '#3B82F6', '#10B981', '#F59E0B', '#EF4444', '#EC4899', '#F43F5E', '#6366F1'];
  ...
  return (
    <section className="bg-white rounded-xl shadow-md max-w-4xl mx-auto mt-8 p-6">
      <h2 className="text-2xl font-semibold mb-6 text-center text-purple-800">Reservas por Grupo de Personas</h2>
...
export default ReservasPorGrupo;
